...
function svq_video_shortcode_output($output, $attr, $content, $instance){
	...
	if (isset($svq_options['svq_active']) ) {
		...
		foreach ( array( 'poster', 'loop', 'autoplay', 'muted' ) as $a ) {
		...
		if ($svq_instance === 1){
			wp_localize_script( 'wp-mediaelement', '_wpmejsSettings',
				array(
					...
					'features' => array('playpause', 'current', 'progress', 'duration', 'volume', 'fullscreen', 'sourcechooser', 'svqinfooverlay', 'svqplaylist', 'svqembed', 'tracks')
				)
			);
